            ...
            <div class="container">
                ...
                if($_GET){
                    ...
                    // echo '<script>alert("'.  $_POST['name'] . ',  '.$_POST['email'].'")</script>';
                        $sql = "select count(e.first_sc_field_id) cnt, 
                                        (
                                        select name_of_sc_f
                                        from scientific_field
                                        where sc_field_id = e.first_sc_field_id
                                        ) first_sc_field_name, 
                                        (
                                        select name_of_sc_f
                                        from scientific_field
                                        where sc_field_id = e.second_sc_field_id
                                        ) second_sc_field_ΝΑΜΕ
                                from (
                        ...
                        if (mysqli_num_rows($result) > 0) {
                          ...
                          while($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo '  <tr>
                            <th scope="row">'.$idx.'</th>
                            <td>'.$row["first_sc_field_name"].'</td>
                            <td>'.$row["second_sc_field_ΝΑΜΕ"].'</td>
                            <td>'.$row["cnt"].'</td>
                            </tr>';
                            $idx +=1;
                          }        
