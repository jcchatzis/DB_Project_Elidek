...
--
-- Host: localhost:8889
-- Generation Time: May 31, 2022 at 08:20 PM
...
CREATE TABLE `researcher` (
  ...
  `researcher_sex` char(2) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
ALTER TABLE `program_belongs_to_admin`
  ADD PRIMARY KEY (`admin_id`,`progam_name`),
...
ALTER TABLE `scientific_field_project`
  ADD PRIMARY KEY (`project_title`,`name_of_sc_f`),
  ADD KEY `name_of_sc_f` (`name_of_sc_f`);
...
ALTER TABLE `telephones`
  ...
  ADD KEY `index_phone` (`abbreviation`);
...
--
ALTER TABLE `Project_donation`
  ADD CONSTRAINT `project_donation_ibfk_1` FOREIGN KEY (`project_title`) REFERENCES `Project` (`project_title`);

--
-- Constraints for table `project_managed_by_org`
--
ALTER TABLE `project_managed_by_org`
  ADD CONSTRAINT `project_managed_by_org_ibfk_1` FOREIGN KEY (`abbreviation`) REFERENCES `organisation` (`abbreviation`),
  ADD CONSTRAINT `project_managed_by_org_ibfk_2` FOREIGN KEY (`project_title`) REFERENCES `Project` (`project_title`);
