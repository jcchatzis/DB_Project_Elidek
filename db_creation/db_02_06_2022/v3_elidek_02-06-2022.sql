...
CREATE TABLE Project(
    ...
    CONSTRAINT CHK_END_DATE CHECK (end_date is null or (end_date >= start_date + INTERVAL 1 YEAR and end_date <= start_date + INTERVAL 4 YEAR))
    );
...
ADD CONSTRAINT CHK_END_DATE CHECK (end_date is null or (end_date >= start_date + INTERVAL 1 YEAR and end_date <= start_date + INTERVAL 4 YEAR));
*/
...
CREATE TRIGGER Project_ins BEFORE INSERT ON Project
       FOR EACH ROW SET @duration = ceil(datediff(end_date, start_date)/365);
...
BEGIN
  declare rowcount int;
  if (
        (new.category = 'UNIVERSITY' and not(new.private_budget is null and new.private_capital is null and new.estimate_budget is not null and new.estimate_budget > 0)) or
        (new.category = 'RESEARCH CENTER' and not(new.private_budget is not null and new.private_budget > 0 and new.private_capital is null and new.estimate_budget is not null and new.estimate_budget > 0)) or
        (new.category = 'COMPANY' and not(new.private_budget is null and new.private_capital is not null and new.private_capital > 0 and new.estimate_budget is null))
     )
  then
...
CREATE TABLE employer_manages_project (
    ...
    org_id char(20) not null,
    CONSTRAINT PK_EMPLOYER_MANAGERS_PROJECT PRIMARY KEY (employer_id, org_id),
    CONSTRAINT FK_EMP_MAN_PRO_ORG_ID FOREIGN KEY (org_id) REFERENCES organisation(org_id),
...
CREATE TABLE researcher_works_org (
    ...
    CONSTRAINT CHK_END_DATE_WORK CHECK (end_date_of_work is null or end_date_of_work >= start_date_of_work)
);
/*
ALTER TABLE researcher_works_org
ADD CONSTRAINT CHK_END_DATE_WORK CHECK (end_date_of_work is null or end_date_of_work >= start_date_of_work);
*/
/*create table review */
CREATE TABLE review(
    researcher_id int not null,
...
CREATE TABLE scientific_field (
    sc_field_id int not null AUTO_INCREMENT,
